...
function getFullDays() {
	...
	$.ajax({
		url: "http://www.louvre.thomassebert.com/Symfony/web/tickets/getFullDays",
		method: "post"
	}).done(function(msg){
		return msg['dates'];	
	});
...
function getRemainingTickets($date) {
	...
	$.ajax({
				url: "http://www.louvre.thomassebert.com/Symfony/web/tickets/getRemainingTickets",
				method: "post",
...
function calendarPartSubmit() {
	...
	$.ajax({
		url: "http://www.louvre.thomassebert.com/Symfony/web/tickets/calendarPartSubmit",
		method: "post",
		data: JSONvalues
	}).done(function(msg){
		if(msg['message'] !== 0)
...
function addTicketForm() {
	...
	$('.empty-cart').hide();
	$('.ticketsList').append('<li class="ticketli">Billet '+number+' - <span class="priceTicket"></span> €');
	...
	$.each(ticketForms, function(i, ticketForm){
		...
		deleteButton.click(function(){
			...
			$.each(getTicketForms, function(i, ticketsForm){
				...
				$('.ticketsList').append('<li id="'+i+'" class="ticketli">Billet '+(i+1)+' - <span class="priceTicket"></span> €');
			});
...
function proceedPayment($token) {
	$.ajax({
				url: "http://www.louvre.thomassebert.com/Symfony/web/tickets/proceedPayment",
				method: "post",
